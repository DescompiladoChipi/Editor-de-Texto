...
def iniciar_editor():
 ...
 #Limpia el área de texto y establece el título por defecto
def nuevo():
    texto.delete(1.0, tk.END)
    root.title("Editor de Texto - Sin título")

# Funcion abrir, abre un archivo existente y carga su contenido en el editor
def abrir():
    ...
    if archivo:
        try:
            # Intenta abrir con codificación UTF-8 (para caracteres especiales)
            with open(archivo, "r", encoding="utf-8") as f:
                contenido = f.read()
        except:
            # Si falla UTF-8, intenta con Latin-1
            with open(archivo, "r", encoding="latin-1") as f:
                # Reemplaza el contenido actual con el del archivo
                contenido = f.read()
        # Actualiza el título con el nombre del archivo
        texto.delete(1.0, tk.END)
        texto.insert(tk.END, contenido)
        # Guarda la ruta del archivo actual
...
def guardar():
    global archivo_actual
    # Si no hay archivo actual, abre diálogo "Guardar como"
...
def guardar_como():
    # Diálogo para especificar nombre y ubicación del archivo
    global archivo_actual
...
def buscar():
    # Crea una ventana emergente para búsqueda
    ...
    resultado.pack()
    # Ejecuta la búsqueda y resalta los resultados
    def ejecutar_busqueda():
        ...
        contador = 0
        while pos:
            # Calcular el final de la coincidencia
            ...
            contador += 1
        # Configurar estilo del resaltado
        texto.tag_config("encontrado", background="#8c9bb8")
        # Mostrar resultados
        resultado.config(text=f"{contador} coincidencia(s) encontradas")
    # Botones de la ventana de búsqueda
...
def abrir_manual():
    webbrowser.open("https://github.com/DescompiladoChipi/Editor-de-Texto/blob/main/ManualDeUsuario.md")
...
def crear_editor():
    ...
    # Variable global para trackear el archivo actual
    archivo_actual = None
    ...
    # Área de texto principal con barra de desplazamiento
    ...
    # Comandos estándar del sistema operativo
...
# Boton para acceder al editor
tk.Button(splash_frame,text="Acceder",font=("Segoe UI", 12, "bold"),width=15,bg="#101a29",fg="#d8dce7",activebackground="#112d4d",activeforeground="#d8dce7",relief="flat",cursor="hand2",command=iniciar_editor).pack(pady=10)
